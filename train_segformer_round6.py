"""train_segformer_round6.py — SegFormer-B4, 5-Class Round 6
Classes : 0=Background  1=Fibrin  2=Granulation  3=Callus  4=Necrotic
Changes vs Round 5:
  - เพิ่ม 2 dataset ใหม่ที่เน้นแยก Necrotic vs Fibrin(Slough) โดยเฉพาะ
    (พบ edge case สับสนระหว่างสอง class นี้ตอนทดสอบ generalization ของ Round 5):
      - wound_tissue_v1 ("My First Project", 180 ภาพ) — Slough/Granulation/Callus/Necrotic
      - tissues_segment (50 ภาพ) — slough-tissue/Granulation-tissue/necrotic-tissue
  - Loads from Round 5 best (segformer_b4_5class_r5_best.pth) with strict=True
  - Saves to segformer_b4_5class_r6_best.pth (does NOT overwrite Round 5)
  - Per-dataset val Dice printed each epoch for easier debugging
  - history saved to history_5class_r6.json
"""
...
# ใหม่ใน Round 6: เสริม Necrotic vs Fibrin(Slough) โดยเฉพาะ
# (แปลงด้วย convert_wound_tissue_v1.py ก่อนรันเทรน)
WOUND_TISSUE_BASE  = Path(r"C:\VITA_Round3\datasets\wound_tissue_v1")
WOUND_TISSUE_TRAIN = WOUND_TISSUE_BASE / "train"
WOUND_TISSUE_VALID = WOUND_TISSUE_BASE / "valid"

# ใหม่ใน Round 6: เสริม Necrotic vs Fibrin(Slough) โดยเฉพาะ (dataset เล็กแต่สะอาด)
# (แปลงด้วย convert_tissues_segment.py ก่อนรันเทรน)
TISSUES_SEG_BASE  = Path(r"C:\VITA_Round3\datasets\tissues_segment")
TISSUES_SEG_TRAIN = TISSUES_SEG_BASE / "train"
TISSUES_SEG_VALID = TISSUES_SEG_BASE / "valid"

SAVE_DIR = Path(__file__).resolve().parent / "checkpoints"
SAVE_DIR.mkdir(exist_ok=True)

# Load from Round 5 best; save to separate R6 file so R5 is preserved
ROUND5_CKPT = SAVE_DIR / "segformer_b4_5class_r5_best.pth"
SAVE_BEST   = SAVE_DIR / "segformer_b4_5class_r6_best.pth"
...
NUM_CLASSES  = 5
EPOCHS       = 50
LR           = 5e-6        # เท่ากับ R5 — ยังไม่ early stop ตอน R5 จบ (best ที่ Ep46/50)
WEIGHT_DECAY = 1e-4        # AdamW decoupled decay
DICE_WEIGHT  = 0.5         # loss = (1-DICE_WEIGHT)*CE + DICE_WEIGHT*Dice

# คง weight เดิมจาก Round 5 (Fibrin/Callus ยังคงต้องการน้ำหนักสูงกว่า BG/Granulation)
...
def train():
    ...
    # wound_tissue_v1 — เสริม Necrotic vs Fibrin(Slough) โดยเฉพาะ
    wound_tissue_train = wound_tissue_val = None
    if (WOUND_TISSUE_TRAIN / "masks").exists():
        wound_tissue_train = NecroticDataset(WOUND_TISSUE_TRAIN, train_tf, weight=8.0)
        wound_tissue_val   = NecroticDataset(WOUND_TISSUE_VALID, val_tf,   weight=8.0)
        datasets.append(wound_tissue_train)
        weights  += wound_tissue_train.sample_weights
        print(f"WoundTissueV1: {len(wound_tissue_train)}")
    else:
        print("[!] wound_tissue_v1 masks not found — run convert_wound_tissue_v1.py first, skipping")

    # tissues_segment — dataset เล็กแต่สะอาด เสริม Necrotic vs Fibrin
    tissues_seg_train = tissues_seg_val = None
    if (TISSUES_SEG_TRAIN / "masks").exists():
        tissues_seg_train = NecroticDataset(TISSUES_SEG_TRAIN, train_tf, weight=8.0)
        tissues_seg_val   = NecroticDataset(TISSUES_SEG_VALID, val_tf,   weight=8.0)
        datasets.append(tissues_seg_train)
        weights  += tissues_seg_train.sample_weights
        print(f"TissuesSeg  : {len(tissues_seg_train)}")
    else:
        print("[!] tissues_segment masks not found — run convert_tissues_segment.py first, skipping")
    ...
    if wound_tissue_val is not None:
        val_parts.append(wound_tissue_val)
    if tissues_seg_val is not None:
        val_parts.append(tissues_seg_val)
    ...
    print(f"DFU val     : {len(dfu_val)}, Necrotic val: {len(necro_val)}"
          ...
          + (f", WoundTissueV1 val: {len(wound_tissue_val)}" if wound_tissue_val else "")
          + (f", TissuesSeg val: {len(tissues_seg_val)}" if tissues_seg_val else ""))
    ...
    wound_tissue_loader = (DataLoader(wound_tissue_val, batch_size=BATCH_SIZE, shuffle=False, num_workers=2)
                         if wound_tissue_val else None)
    tissues_seg_loader = (DataLoader(tissues_seg_val, batch_size=BATCH_SIZE, shuffle=False, num_workers=2)
                         if tissues_seg_val else None)
    ...
    if ROUND5_CKPT.exists():
        print(f"[Round 6] Loading Round 5 best → {ROUND5_CKPT.name}")
        model.load_state_dict(torch.load(ROUND5_CKPT, map_location=device), strict=True)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        wound_d = eval_loader(model, wound_tissue_loader, device) if wound_tissue_loader else None
        tseg_d  = eval_loader(model, tissues_seg_loader, device) if tissues_seg_loader else None
        ...
        log = {
            ...
            "per_dataset": {
                ...
                **({"wound_tissue_v1": [round(v, 4) for v in wound_d]} if wound_d else {}),
                **({"tissues_segment": [round(v, 4) for v in tseg_d]} if tseg_d else {}),
            },
        }
        history.append(log)
        ...
        print(f"         DFU  Necro={dfu_d[4]:.4f}↓  "
              ...
              + (f"  WoundV1 Necro={wound_d[4]:.4f}/Fib={wound_d[1]:.4f}" if wound_d else "")
              + (f"  TSeg Necro={tseg_d[4]:.4f}/Fib={tseg_d[1]:.4f}" if tseg_d else ""))
        ...
        if epoch % 10 == 0:
            torch.save(model.state_dict(), SAVE_DIR / f"segformer_b4_5class_r6_ep{epoch}.pth")

    with open(SAVE_DIR / "history_5class_r6.json", "w") as f:
